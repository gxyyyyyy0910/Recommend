...
export default {
  ...
  methods: {
    ...
    async order(username){
       //let rate = await this.initRate(username)
      this.initRate(username)
      // let value = await this.initValue(username)
      // let rate = this.$axios.get('/user/createAvgList/'+username);
      // let value = this.$axios.get('/user/createRadar/' + username);
      // rate.then((res) => {
      //   //younghoo
